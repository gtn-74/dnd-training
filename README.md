...
```
'use client'
import SideBar from '@/components/common/global-header'
...
import { Container, Item } from '../../components/audition/cast/foo/index'
import AuditionSideBar from '@/components/audition-sideBar'
...
export default function UserPage() {
  ...
  // CSRにスイッチ // 'use client'が書いてあっても、初回はサーバーサイドでレンダリングするっぽい
  const [isClient, setIsClient] = React.useState<boolean>(false)
  React.useEffect(() => {
    setIsClient(true) // クライアントサイドでのみisClientをtrueに設定
  }, [])

  // isClientがtrueになるまでレンダリングは発生しない。trueになったらレンダリングが発生する
  if (!isClient) {
    return null
  }
